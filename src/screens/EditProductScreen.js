...
import { Text, TextInput, Button, HelperText, Card, Divider, ActivityIndicator } from 'react-native-paper';
...
export default function EditProductScreen({ route, navigation }) {
  ...
  const onSave = async (values) => {
    try {
      ...
      if (imageUri) {
        ...
        const newPath = `products/${uuidv4()}.${ext}`;
        const uploaded = await uploadImageFromUri(imageUri, newPath, `image/${ext === 'jpg' ? 'jpeg' : ext}`);
        if (imagePath) {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: '#fff' }} contentContainerStyle={{ padding: 16, paddingBottom: 100 }}>
      <Text variant="headlineMedium" style={{ fontWeight: '700', marginBottom: 12 }}>Edit Product</Text>
      ...
      <Card style={{ borderRadius: 16, padding: 12 }}>
        <Card.Content>
          ...
          {existing?.imageUrl ? (
            <Image source={{ uri: existing.imageUrl }} style={{ width: '100%', height: 160, borderRadius: 12, marginBottom: 8 }} />
          ) : null}
          ...
          {imageUri ? (
            <Image source={{ uri: imageUri }} style={{ width: '100%', height: 160, borderRadius: 12, marginTop: 8 }} />
          ) : null}

          <Button style={{ marginTop: 12 }} mode="contained" onPress={handleSubmit(onSave)} loading={saving} disabled={saving}>
            Save Changes
          </Button>
        </Card.Content>
      </Card>
